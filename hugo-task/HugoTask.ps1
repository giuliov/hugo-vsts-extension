...
try {
    ...
    [bool]$minify = Get-VstsInput -Name EnableMinification -AsBool
    ...
    Invoke-Hugo -hugoExePath $hugoExePath -source $source -destination $destination -baseURL $baseURL -buildDrafts $buildDrafts -buildExpired $buildExpired -buildFuture $buildFuture -uglyURLs $uglyURLs -minify $minify
